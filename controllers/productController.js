...
export const createProductController=async(req,res)=>{
    try{
        ...
        if(photo && photo.size>100000){
            return res.status(500).send({error:'photo is required and photo should be less than 1 MB'})
        }
...
export const braintreePaymentController = async (req, res) => {
    try {
        ...
        const newTransaction = gateway.transaction.sale({
            ...
            if (error) {
                return res.status(500).json({ error: 'Transaction failed', details: error });
            }
            ...
            if (result) {
                try {
                    ...
                    res.status(500).json({ error: 'Order creation failed', details: orderError });
                }
            } else {
